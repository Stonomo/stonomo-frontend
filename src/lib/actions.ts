...
import { modifyProfile } from "../routes/users";
...
export async function profileAction({ request }: { request: Request }) {
	const formData = await request.formData()
	const name = formData.get('facilityName') as string
	...
	return await modifyProfile(name, phone, email, addrSt1, addrSt2, addrCity, addrState, addrZip)
}
